...
class OBJLoader {
    ...
    load( gl ) {
        ...
        // Push each vertex and normal from the appropriate lists to vertex_data
        // they come in sets of three, corresponding to the x, y, and z coordinates of each vertex/normal
        // In this implementation, position, normal, tangent, and tex coord values are interleafed
        // in other words, all the information for one vertex is loaded contiguously, then all the information for the next, etc
        for (let i = 0, j = 0; i < vertex_positions.length; i += 3, j += 2){
            ...
            // tangents and tex coords are only added if they exist
            // since tex coords come in pairs instead of triples, use a separate counter j to keep track of them
            if (vertex_tangents.length > 0 && vertex_texture_coords.length > 0){
                vertex_data.push(vertex_tangents[i])
                vertex_data.push(vertex_tangents[i+1])
                vertex_data.push(vertex_tangents[i+2])
                vertex_data.push(vertex_texture_coords[j])
                vertex_data.push(vertex_texture_coords[j+1])
            }
		}
